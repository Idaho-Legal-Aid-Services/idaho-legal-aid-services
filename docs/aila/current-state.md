...
| GA4 tag and live rate-limit override | No GA loader or `dataLayer` bootstrap rendered in sampled local `/assistant` HTML; runtime truth now expects assistant-page GA suppression explicitly | Sampled `dev`/`test` still show no assistant-page GA loader; hosted `live` pre-deploy still renders `googletagmanager`, `window.dataLayer`, and `gtag()` on `/assistant`, while non-assistant `live` pages remain the only allowed sitewide GA surface after TOVR-15 | `settings.php` still scopes `google_tag_id` to `live`, but theme preprocessing now suppresses GA bootstrap on route `ilas_site_assistant.page` and runtime truth distinguishes sitewide tag presence from assistant-page expectations | [^CLAIM-099][^CLAIM-190][^CLAIM-243][^CLAIM-244] |
...
| Pinecone secret wiring | Safe runtime check showed Pinecone key present in local effective config | TOVR-09 reruns showed Pinecone key present in `dev`/`test`/`live` effective config, and hosted `ilas:runtime-truth` is now available there | Secret wiring is active even though vector retrieval remains disabled | [^CLAIM-068][^CLAIM-189][^CLAIM-190][^CLAIM-216] |
...
| Langfuse runtime export | Observability | `confirmed partial` | Queue-backed trace/span/event export | `local`, `dev`, `test`, `live` | Safe runtime booleans show enabled effective config; TOVR-04 proves fresh local direct and queued traces in Langfuse API with privacy-safe payload summaries; TOVR-13 now adds vector/lexical retrieval metadata to repo-side trace payloads for future live diagnostics | `high` | Deploy the TOVR-13 trace-contract change and prove one hosted live request shows the new vector fields in Langfuse | [^CLAIM-189][^CLAIM-190][^CLAIM-210][^CLAIM-237] |
...
| GA4 / `dataLayer` | Analytics | `confirmed partial` | Sitewide GA bootstrap may remain active on non-assistant `live` pages, but assistant-originated widget telemetry is denied from GA/dataLayer | Repo and local runtime now suppress assistant-page GA bootstrap and remove widget `dataLayer` pushes; hosted `live` `/assistant` remains pre-deploy until the next release | Local `/assistant` rendered only Sentry markers after cache clear, while hosted `live` `/assistant` still showed old GA markers and hosted `live` `/` still showed sitewide GA markers | `medium` | Keep deny-by-default assistant GA policy and verify `/assistant` stays GA-suppressed after each deploy | [^CLAIM-027][^CLAIM-190][^CLAIM-243][^CLAIM-244][^CLAIM-245] |
...
| Search API vector indexes | Retrieval infrastructure | `lexical-first retrieval enabled on non-live under runtime controls` | Provide optional Pinecone-backed FAQ/resource vector indexes | Repo config present; runtime overrides now set `vector_search.enabled=true` on `local`, `dev`, and `test`, while `live` remains hard-forced off | TOVR-12 repaired local Pinecone parity, restored successful local direct vector queries, enabled `dev` / `test` without exported-config drift, and rechecked hosted runtime-truth plus smoke behavior. TOVR-13 confirms the live infrastructure itself is healthy (`faq_accordion_vector` `883/883`, `assistant_resources_vector` `152/152`, retrieval/hygiene `healthy`), but the rollout decision is still blocked by prompt-quality proof, diagnostics-token absence, the failing latest `master` quality gate, and embeddings-timeout separation. | `high` | Live remains intentionally disabled after TOVR-13; exact prerequisites are documented in `runtime/tovr-13-pinecone-live-readiness.txt`. | [^CLAIM-066][^CLAIM-067][^CLAIM-216][^CLAIM-221][^CLAIM-222][^CLAIM-223][^CLAIM-224][^CLAIM-225][^CLAIM-226][^CLAIM-227][^CLAIM-228][^CLAIM-229][^CLAIM-230][^CLAIM-231][^CLAIM-232][^CLAIM-233][^CLAIM-235][^CLAIM-238] |
