...
5. Live rollout remains runtime-toggle controlled; exported config stays conservative and live vector search is now secret + cache-clear controlled instead of hard-disabled in repo code.
...
4. Live enablement remains governed by rollout readiness, not by hosted secret/index absence: TOVR-12 proves `local` / `dev` / `test` can be runtime-enabled under lexical-first safeguards, TOVR-13 preserves the historical blocked live gate, and TOVR-17 refreshes the repo contract so `live` is runtime-toggle controlled once the code is deployed. (Refs: current-state §1, §8; evidence-index CLAIM-220, CLAIM-229, CLAIM-230, CLAIM-232, CLAIM-235, CLAIM-238; runbook §3)

### TOVR-12 Pinecone non-live enablement disposition (2026-03-17)
1. Runtime-only Pinecone enablement is now proven on `local`, `dev`, and `test`: sync config remains `vector_search.enabled=false`, but runtime truth shows stored `false` versus effective `true` on all three non-live environments while `live` remained forced off in the historical TOVR-12 pass. (Refs: current-state §1, §8; evidence-index CLAIM-229, CLAIM-230, CLAIM-231; runbook §3)
...
4. The historical live gate remained closed in TOVR-12. TOVR-13 later records that blocked production decision, and TOVR-17 now supersedes the repo contract by allowing live runtime toggling without changing exported config or the live admin form. (Refs: current-state §1, §8; evidence-index CLAIM-227, CLAIM-228, CLAIM-232, CLAIM-233, CLAIM-234, CLAIM-238; runbook §3, §4)
...
3. The final TOVR-13 verdict is `Blocked with explicit evidence`, not `Ready for live enablement`: the latest `master` Quality Gate run `23218168501` is still red, positive machine-auth diagnostics remain unproven because the live diagnostics token is absent, prompts 2 / 3 still lack accepted vector-quality proof, and embeddings-side timeout governance is still shared/global. This disposition is preserved as the historical blocked baseline and is superseded by TOVR-17 for the current repo contract. (Refs: current-state §1, §8; evidence-index CLAIM-233, CLAIM-234, CLAIM-238; runbook §3, §4)
4. Exact prerequisites remain relevant to hosted rollout proof even after the contract refresh: green replacement quality gate, live diagnostics-token proof or a formally adopted authenticated drush-monitoring standard, deploy and hosted proof for the new vector Langfuse metadata, accepted prompt-level quality scope, embeddings-timeout separation, and a documented runtime-only rollback sequence for any future live toggle. (Refs: current-state §8; evidence-index CLAIM-233, CLAIM-237, CLAIM-238; runbook §3, §4)

### TOVR-17 Pinecone live enablement disposition (2026-04-21)
1. The repo contract now makes `live` eligible for the existing runtime-only `ILAS_VECTOR_SEARCH_ENABLED` toggle. `settings.php` no longer hard-forces `vector_search.enabled=false` on `live`, while exported config stays conservative and the live admin form still cannot persist `true`.
2. Live rollout remains operator-driven rather than config-driven: enablement and rollback stay secret + cache-clear controlled, and `ilas:runtime-truth` plus `ilas:vector-status` are the canonical verification surfaces.
3. Vector-index hygiene refresh now treats current config as authoritative for managed index IDs and expected Pinecone metadata, so stale stored snapshot state can no longer preserve legacy `server_id`, `dimensions`, or other expected contract values after refresh.
4. Hosted live enablement is still deployment-bound. The refreshed repo contract must still be deployed to Pantheon `live`, then verified with the `ILAS_VECTOR_SEARCH_ENABLED=1` path, smoke/probe commands, and an explicit rollback pass before the runtime posture can be considered proven.
