...
**Update, later the same day:** rows 3, 4 and 6 are resolved — everything below was committed and
deployed Dev → Test → Live as master `20b18a47e1`, which shipped items 6 and 14. See the tracker.
...
| **Closed** | FU-4 — Test DB has 0 Spanish nodes | **Closed 2026-07-30** — the test deploy used `--sync-content`; test has 99 Spanish nodes again | 99 on dev, 0 on test. Test cannot validate any multilingual behaviour, which is why item 1's Spanish verification had to run on Dev and Live. | Refresh Test from Live | You (or Claude with approval) | `SELECT COUNT(*) FROM node_field_data WHERE langcode='es'` on test | Non-zero, and `/es` resolves to an aliased Spanish page |
