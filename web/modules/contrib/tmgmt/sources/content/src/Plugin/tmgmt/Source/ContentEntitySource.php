...
class ContentEntitySource extends SourcePluginBase implements SourcePreviewInterface, ContinuousSourceInterface {
  ...
  protected function doSaveTranslations(ContentEntityInterface $entity, array $data, $target_langcode, JobItemInterface $item, $save = TRUE) {
   // If the translation for this language does not exist yet, initialize it.
    ...
    if ($save && $entity->getEntityType()->isRevisionable()) {
      ...
      if ($translation->id() && $storage instanceof TranslatableRevisionableStorageInterface) {
    ...
    foreach ($embeddable_fields as $field_name => $field_definition) {
      ...
      $is_asymmetric_translation_mode = $target_type == 'paragraph' && $field->getFieldDefinition()->isTranslatable();
      ...
      // source language.
      if (!$is_target_type_translatable || $is_asymmetric_translation_mode) {
   ...
   * @param bool $is_asymmetric_translation_mode
   *   (optional) Whether paragraph asymmetric mode is enabled.
   *
   * @return \Drupal\Core\Entity\ContentEntityInterface|null
   */
  protected function findReferencedEntity(FieldItemListInterface $field, array $field_item, $delta, $property, $is_target_type_translatable = TRUE, $is_asymmetric_translation_mode = FALSE) {
    ...
    if (isset($field_item[$property]['#id'])) {
      foreach ($field as $item_delta => $item) {
        if ($item->$property instanceof ContentEntityInterface) {
          ...
          if ($referenced_entity->id() == $field_item[$property]['#id'] || ($item_delta === $delta && (!$is_target_type_translatable || $is_asymmetric_translation_mode))) {
