...
if (!defined('CIVICRM_UF_DSN') && CIVICRM_UF !== 'UnitTests') {
  if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
    // On Pantheon, use the same database as Drupal
    if (isset($_SERVER['PRESSFLOW_SETTINGS'])) {
      $pressflow_settings = json_decode($_SERVER['PRESSFLOW_SETTINGS'], TRUE);
      if (isset($pressflow_settings['databases']['default']['default'])) {
        $db = $pressflow_settings['databases']['default']['default'];
        $dsn = sprintf('mysql://%s:%s@%s:%s/%s?new_link=true',
          $db['username'],
          $db['password'],
          $db['host'],
          $db['port'],
    ...
    // Fallback if PRESSFLOW_SETTINGS not available
    if (!defined('CIVICRM_UF_DSN')) {
      define('CIVICRM_UF_DSN', 'mysql://pantheon:@dbserver.dev.site-id.drush.in:port/pantheon?new_link=true');
    }
  }
  else {
...
$civicrm_paths['civicrm.files']['url'] = $base_url . '/sites/default/files/civicrm';
...
$civicrm_paths['civicrm.root']['url'] = $base_url . '/modules/contrib/civicrm';
$civicrm_setting['domain']['userFrameworkResourceURL'] = $base_url . '/libraries/civicrm/';
...
if (!defined('CIVICRM_DSN')) {
  ...
  elseif (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
    ...
    // Fallback if PRESSFLOW_SETTINGS not available
    if (!defined('CIVICRM_DSN')) {
      define('CIVICRM_DSN', 'mysql://pantheon:@dbserver.dev.site-id.drush.in:port/pantheon?new_link=true');
    }
  }
  else {
    // Local development (DDEV)
...
// Dynamically determine the civicrm root path
$base_path = defined('DRUPAL_ROOT') ? DRUPAL_ROOT : realpath(__DIR__ . '/../..');
$project_root = realpath($base_path . '/..');
$civicrm_root = $project_root . '/vendor/civicrm/civicrm-core/';
if (!defined('CIVICRM_TEMPLATE_COMPILEDIR')) {
  $base_path = defined('DRUPAL_ROOT') ? DRUPAL_ROOT : realpath(__DIR__ . '/../..');
  define('CIVICRM_TEMPLATE_COMPILEDIR', $base_path . '/sites/default/files/civicrm/templates_c');
}
...
if (!defined('CIVICRM_UF_BASEURL')) {
  define('CIVICRM_UF_BASEURL', 'https://ilas.ddev.site');
}

// Force correct resource URL regardless of how site is accessed
...
if (!defined('CIVICRM_SMARTY_AUTOLOAD_PATH') && getenv('CIVICRM_SMARTY_AUTOLOAD_PATH') === FALSE) {
  // For composer-based installs, Smarty is in civicrm-packages, not civicrm-core/packages
  if (is_dir($civicrm_root . '/../civicrm-packages')) {
    define('CIVICRM_SMARTY_AUTOLOAD_PATH', $civicrm_root . '/../civicrm-packages/smarty5/Smarty.php');
  }
  elseif (is_dir($civicrm_root . '/packages')) {
...
if (!defined('CIVICRM_SITE_KEY')) {
  define('CIVICRM_SITE_KEY', 'abf417c54f53ac4f832c329b1d091f19');
}
...
if (!defined('CIVICRM_CRED_KEYS')) {
  define('_CIVICRM_CRED_KEYS', 'aes-cbc:hkdf-sha256:9HcejIOXPe1PDVumtyma3OQDzB47LUay9K9Tn9yXplceNSXEgw');
...
if (!defined('CIVICRM_DEPLOY_ID')) {
  define('_CIVICRM_DEPLOY_ID', 'Ibijh1sXk1YihA');
...
if (!defined('CIVICRM_SIGN_KEYS')) {
  define('_CIVICRM_SIGN_KEYS', 'jwt-hs256:hkdf-sha256:0ROOy1WsHv8I5SC8gFmPbpLVZWM13xb1xmEbNXJFLX863A1F1iu7w');
